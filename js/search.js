...
// 原来的，现在使用小陈gpt💕增加  720yun.com 的搜索引擎，则在搜索时添加 /0/1  并添加回车事件
...
$(".search-btn").click(function() {
    ...
    if (thisSearch === searchUrl) {
        // 如果是 720yun.com 的搜索引擎，则在搜索时添加 /0/1
        // window.open(thisSearch + encodeURIComponent($("#txt").val()) + "/0/1?=小陈gpt");
        window.open(thisSearch + encodeURIComponent($("#txt").val()) + "/0/1?from=1nav.ml");
    } else {
        // 否则直接跳转
        window.open(thisSearch + $("#txt").val());
    }
    ...
    var search = {
        data: [{
            ...
            },
            {
                ...
                name: 'VR全景',
                icon: 'https://3o.hk/images/2023/03/30/VRAR.png',
                color: '#148aff',
