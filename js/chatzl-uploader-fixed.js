        ...
        ! function () {
            ...
            function H(t) {
                ...
                t.msg = t.msg.replace(/【(.*?)】/g, function (match, p1) {
                    if (p1.includes('blob') || p1.includes('comfy')) {
...
//         MARKDOWN_TEMPLATE: "{url}", // ✅ 会被生效 ![{filename}]({url})
//         AUTO_COPY_URL: true,
//         ALLOWED_HOSTS: ["z-l.top", "127.0.0.1"], // ✅ 会被生效
...
//         if (host === "*") return true;
//         if (host.startsWith("*.")) {
//             return currentHost.endsWith(host.slice(1));
//         }
//         return currentHost === host;
//     });

//     if (!isAllowed) {
//         console.warn(`[ImageUploader] 当前域名 ${currentHost} 不在允许的 ALLOWED_HOSTS 列表中，终止运行。`);
//         return;
//     }
//     // ✅ 添加样式
//     GM_addStyle(`
...
//                 }
//             },
//             onerror: () => showNotification("上传失败（网络错误）", "error"),
//         });
//     }
...
//         AUTO_COPY_URL: true,
//         ALLOWED_HOSTS: ["z-l.top", "127.0.0.1"],
//         MAX_FILE_SIZE: 5 * 1024 * 1024,
//     };
...
//             warningDiv.innerHTML = `当前域名 ${currentHost} 不在白名单，去留言添加 <a href="https://blog.z-l.top/posts/chat.html" target="_blank">https://blog.z-l.top/posts/chat.html</a>`;
...
//             // 点击链接或关闭弹窗
//             warningDiv.querySelector("a").addEventListener("click", () => {
//                 window.open("https://blog.z-l.top/posts/chat.html", "_blank");
//             });

//             setTimeout(() => warningDiv.remove(), 5000); // 5秒后自动移除弹窗
//             return;
//         }

//         const filename = `image-${Date.now()}`; //后缀.png可以改
//         const formData = new FormData();
//         formData.append("file", blob, filename);
...
//         ENABLE_MANUAL_UPLOAD: true,
//         UPLOAD_TRIGGER_SELECTOR: "#cfbed",
//         MARKDOWN_TEMPLATE: "{url}1", // 插入原始链接
//         AUTO_COPY_URL: true,
//         ALLOWED_HOSTS: ["z-l.top", "127.0.0.1"],
//         MAX_FILE_SIZE: 5 * 1024 * 1024,
//     };
...
//             const textarea = document.querySelector(".ctrm-textarea");
//             if (textarea) {
//                 const rect = textarea.getBoundingClientRect();
//                 preview.style.top = (window.scrollY + rect.top - 120) + "px";
//                 preview.style.left = (window.scrollX + rect.left + 10) + "px";
...
//             const warningDiv = document.createElement("div");
//             warningDiv.classList.add("upload-warning");
//             warningDiv.innerHTML = `当前域名 ${currentHost} 不在白名单，去留言添加 <a href="https://blog.z-l.top/posts/chat.html" target="_blank">点击添加</a>`;
//             document.body.appendChild(warningDiv);
//             warningDiv.style.display = "block";
